...
import Ros.Internal.DepFinder (findMessages, findDepsWithMessages, hasMsgs, findServices)
...
buildNewPkgMsgs tools fname =
  do liftIO . putStrLn $ "Generating package " ++ fname
     ...
     let pkgMsgs' = map (B.pack . cap . dropExtension . takeFileName) pkgMsgs
         ...
         --TODO: use the names generated by parse
         names = map ((destDir </>)
     ...
     --TODO: check parsedSrvs for errors
     parsedSrvs <- liftIO $ mapM parseSrv pkgSrvs
     --TODO: better error message
     let parsedSrvs' = either (error "Could not parse service") id <$> parsedSrvs
     ...
     --TODO: should be hasMsgs or hasSrvs
    ...
    where err pkg = error $ "Couldn't parse message " ++ pkg
          ...
          isLeft (Left _) = True
          isLeft _ = False
          --TODO: update genMsgCabal with services
